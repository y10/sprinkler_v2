...
class AsyncCallbackWebHandler: public AsyncWebHandler {
  ...
  public:
    ...
    void setUri(const String& uri){ 
      ...
      _isBraces = uri.indexOf("{") != -1 && uri.indexOf("}") != -1;
    }
      ...
      if(_isBraces)
      {
        ...
        for (unsigned int i = 0; i < uriLength; i++, requestUriIndex++) {
          ...
          if (i >= uriLength) {
              ...
              request->_addPathParam(requestUri.substring(requestUriIndex).c_str());
              //return request->_pathParams.back().indexOf("/") == -1; // path argument may not contain a '/'
              return true;
          }
          else
          {
              ...
              if (uriIndex < 0)
                  return false;
              request->_addPathParam(requestUri.substring(requestUriIndex, uriIndex).c_str());
